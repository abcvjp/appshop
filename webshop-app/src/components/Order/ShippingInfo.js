...
const ShippingInfo = ({ customer_name, email, phone_number, address, shipping_note, shipping_method,
	setCustomerName, setEmail, setPhoneNumber, setAddress, setShippingNote, setShippingMethod }) => {
	const classes = useStyles()
	const [shippingMethods, setShippingMethods] = useState([])

	const handleShippingMethodChange = (event) => {
		setShippingMethod(shippingMethods[parseInt(event.target.value) - 1])
	}
	...
	return (
		<div>
			...
			<TextField InputLabelProps={{ shrink: true, color: "primary" }} className={classes.textField} required fullWidth
				key="email" label="EMAIL" variant="outlined" margin="normal"
				value={email}
				onChange={(e) => {
					setEmail(e.target.value)
				}} />

			<TextField InputLabelProps={{ shrink: true }} className={classes.textField} required fullWidth
				key="customer_name" label="FULL NAME" variant="outlined" margin="normal"
				value={customer_name}
				onChange={(e) => {
			...
			<TextField InputLabelProps={{ shrink: true }} className={classes.textField} required fullWidth
				key="phone_number" label="PHONE NUMBER" variant="outlined" margin="normal"
				value={phone_number}
				onChange={(e) => {
			...
			<TextField InputLabelProps={{ shrink: true }} className={classes.textField} required fullWidth
				key="address" label="SHIPPING ADDRESS DETAIL (Street, district, city, ...)" variant="outlined" margin="normal"
				value={address}
				onChange={(e) => {
					setAddress(e.target.value)
				}} />

			<TextField InputLabelProps={{ shrink: true }} className={classes.textField} required fullWidth multiline rows={4}
				key="shipping_note" label="SHIPPING NOTE" variant="outlined" margin="normal"
				value={shipping_note}
				onChange={(e) => {
			...
			{shippingMethods.length > 0 ?
				<RadioGroup aria-label="gender" name="gender1" value={shipping_method.id || ''}
