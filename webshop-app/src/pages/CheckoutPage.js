...
import ReviewAndPayment from '../components/Order/ReviewAndPayment'
import SuccessDialog from '../components/Order/SuccessDialog'
...
const CheckoutPage = () => {
	...
	const handleBack = (data) => {
		setState(state => ({ ...state, ...data, activeStep: state.activeStep - 1 }))
	}

	const handleReset = () => {
		setState(state => ({ ...state, activeStep: 0 }))
	}

	const handlePlaceOrder = () => {
		const { email, customer_name, phone_number, address, shipping_note } = orderInfo
		...
		const order_items = orderItems.current.map(item => {
			const { buy_able, product_slug, product_thumbnail, ...temp } = item
			return temp
		})
		API.post('/order', {
			email, customer_name, phone_number, address, shipping_note, shipping_method_id, payment_method_id,
			order_items
		}).then(response => response.data.result).then(order => {
			setState(state => ({ ...state, isOrderSuccess: true, placedOrder: order }))
		}).catch(err => {
	...
	const setCustomerName = (name) => {
		setOrderInfo(state => ({ ...state, customer_name: name }))
	}

	const setEmail = (email) => {
		setOrderInfo(state => ({ ...state, email }))
	}

	const setPhoneNumber = (phone_number) => {
		setOrderInfo(state => ({ ...state, phone_number }))
	}

	const setAddress = (address) => {
		setOrderInfo(state => ({ ...state, address }))
	}

	const setShippingNote = (shipping_note) => {
		setOrderInfo(state => ({ ...state, shipping_note }))
	}

	const setShippingMethod = (shipping_method) => {
		setOrderInfo(state => ({ ...state, shipping_method }))
	}

	const setPaymentMethod = (payment_method) => {
		setOrderInfo(state => ({ ...state, payment_method }))
	}
	...
	function getStepContent(stepIndex) {
		const { email, customer_name, phone_number, address, shipping_note, shipping_method, payment_method } = orderInfo
		switch (stepIndex) {
			case 0:
				return <ShippingInfo
					email={email}
			...
			<Container className={classes.root} maxWidth="lg">
				...
				<Grid container spacing={3} justifyContent="space-between" alignItems="flex-start">
					...
					<Grid item md={4} style={{ flexGrow: 1 }}>
						<OrderSummary orderItems={orderItems.current}
