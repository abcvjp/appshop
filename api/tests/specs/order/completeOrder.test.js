...
describe('PUT /order/${orderId}/complete', () => {
  // prepare data
  ...
  test('with handling order', async () => {
    ...
    expect(orderAfterUpdate.status).toEqual('Completed');
    expect(orderAfterUpdate.shipping_status).toEqual('Successfully delivered');
    expect(orderAfterUpdate.payment_status).toEqual('Paid');
  });
  ...
  test('without access token', async () => {
    const res = await testClient
      .put(`/order/${sampleOrder.id}/complete`)
      .set('Accept', 'application/json')
      .expect(401)
      .expect('Content-Type', /json/);
  });
